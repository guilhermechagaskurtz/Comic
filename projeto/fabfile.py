...
CONFIG['production'] = {
    'ssh_user':           'alexandre',
    'domain':             'comic.lapinf.com.br',
    'deploy_to':          '/home/alexandre/comic',
    'repository':         'https://github.com/alexandrezamberlan/comic.git',
    'branch':             'master',
    'post_deploy_script': 'source ~/comic/venv/bin/activate && cd ~ && pip install -r ~/comic/requirements.txt && cd ~ && python comic/manage.py migrate && sudo service comic restart && deactivate',
    'post_setup_script':  None
}
...
def setup():
    ...
    if exists(deploy_to):
        ...
        with cd(deploy_to):
            ...
            if CONFIG[DEPLOY_TARGET]['post_setup_script']:
...
def deploy():
    ...
    with cd(deploy_to):
        ...
        if CONFIG[DEPLOY_TARGET]['post_deploy_script']:
