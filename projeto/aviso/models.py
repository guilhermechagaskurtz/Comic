...
class Aviso(models.Model):
    ...
    enviado = models.BooleanField(_('Selecione para enviar aviso por email aos destinatários'), default=False, help_text='Se marcado, aviso é enviado uma vez aos detinatários ativos')
    is_active = models.BooleanField('Selecione para publicar na home dos usuários o aviso', default=True, help_text='Se ativo, o aviso aparece na home de usuários')
    ...
    def get_absolute_url(self):
        return reverse('aviso_update', args=[str(self.id)])

    @property
    def get_delete_url(self):
        return reverse('aviso_delete', args=[str(self.id)])
    
    @property
    def get_envia_email_url(self):
        return reverse('aviso_envia_email', args=[str(self.id)])
